(function () {
  ...
  async function request(capability, context, signal) {
    ...
    try {
      ...
      throw new Error(window.AIProviderLayer.plainLanguageError('Anthropic', null, 'network'));
    }
    let result = {}; try { result = await response.json(); } catch (error) {}
    if (!response.ok) throw new Error(window.AIProviderLayer.plainLanguageError('Anthropic', response.status));
